...
@njit(parallel=True)
def get_Psi_new_grid(Psi, r_grid, r, r0, dQ):
    Nr = r.size
...
@njit(parallel=True)
def get_density(r_xi, vz_xi, dQ, dr, Nr):
    ...
    for i_xi in prange(N_xi):
        ...
        dens_loc = slice_density_projection(dens_loc, r_xi[i_xi], \
            dQ/(1-vz_xi[i_xi]), dr, Nr)
        dens[i_xi,:] = dens_loc / dQ_new

    return dens

@njit(parallel=True)
def get_field(field_val, r_xi, vz_xi, dQ, dr, Nr):
    ...
    for i_xi in prange(N_xi):
        dens_loc = np.zeros(Nr)
        dens_loc = slice_density_projection(dens_loc, r_xi[i_xi], \
            dQ * field_val[i_xi]/(1-vz_xi[i_xi]), dr, Nr)
        dens[i_xi,:] = dens_loc/dQ_new
